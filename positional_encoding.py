...
"""Positional Encoding Modules, some of them are adapted from ESPNet"""
...
def get_sinusoid_encoding_table_numpy(n_position, d_hid, padding_idx=None):
...
def get_sinusoid_encoding_table(n_position: int, d_hid: int, padding_idx: Optional[int] = None):
    """ Sinusoid position encoding table """

    sinusoid_table = torch.tensor([[pos_i / torch.pow(torch.tensor([10000]),
                                    2 * (hid_j // 2) / d_hid).item() \
